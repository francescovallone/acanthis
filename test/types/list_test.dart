    ...
    () {
      ...
      test(
        ...
        () {
          ...
          expect(() => list.parse(['value']), throwsA(TypeMatcher<ValidationError>()));
        }
      );
      ...
      test(
        ...
        () {
          ...
          expect(() => list.parse(['value', 'other']), throwsA(TypeMatcher<ValidationError>()));
        }
      );
      ...
      test(
        ...
        () {
          ...
          expect(() => list.parse(['value', 'other', 'another', 'one']), throwsA(TypeMatcher<ValidationError>()));
        }
      );
      ...
      test(
        ...
        () {
          ...
          expect(() => list.parse(['value', 'other', 'another', 'another']), throwsA(TypeMatcher<ValidationError>()));
        }
      );
      ...
      test(
        ...
        'then the result should be unsuccessful',
        () {
          final list = acanthis.string().min(5).max(20).list().customCheck(onCheck: (value) => value.length == 4, error: '5', name: '5');
          final result = list.tryParse(['value', 'other', 'another', 'another', 'oneee']);

          expect(result.success, false);

          expect(() => list.parse(['value', 'other', 'another', 'another', 'oneee']), throwsA(TypeMatcher<ValidationError>()));
        }
      );
      ...
      test(
        ...
        () {
          final list = acanthis.string().min(5).max(20).list().transform(
            (value) => value.map((e) => e.toUpperCase()).toList() 
          );
